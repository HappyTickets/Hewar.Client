...
export class PriceRequestsService {
  ...
  createPriceRequest(data: ICreatePriceReq) {
    return this.http.post<IResponse<number>>(
      `${this.baseEndPoint}createRequest`,
      data
    );
  }
  updatePriceRequest(data: ICreatePriceReq) {
    return this.http.put<IResponse<unknown>>(
      `${this.baseEndPoint}update-request`,
      data
    );
  }
  cancelPriceRequest(priceRequestId: number) {
    return this.http.patch<IResponse<unknown>>(
      `${this.baseEndPoint}/requests/${priceRequestId}/cancel`,
      {}
    );
  }
  getMyPriceRequestsAsFacility() {
    return this.http.get<IResponse<IFacilityRequest[]>>(
      `${this.baseEndPoint}/requests/facility`
    );
  }
  getMyPriceRequestsAsCompany() {
    return this.http.get<IResponse<ICompanyRequest[]>>(
      `${this.baseEndPoint}/requests/company`
    );
  }
}
